...
```powershell
makensis /DVERSION=0.1.15-preview niss\atow_setup.nsi
```
...
```powershell
& 'C:\Program Files (x86)\NSIS\makensis.exe' /DVERSION=0.1.15-preview niss\atow_setup.nsi
```
...
```text
artifacts\release\0.1.15-preview
```
...
```text
atow-character-creator-0.1.15-preview-<commit>-setup.exe
atow-character-creator-0.1.15-preview-<commit>-setup.exe.sha256
```
